...
- **Goal Updates**: `PATCH /user_goals` replaces the entire `plan_data` document - it does not merge. Before changing any goal (e.g., "bump my carb target to 200g on high-carb days"), first call `GET /user_goals`, take the existing `plan_data` object, apply only the requested change to it in your own reasoning, and send the complete resulting object back as `plan_data`. Never send a partial object - doing so will silently delete every other stored rule.